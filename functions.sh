...
function backup_mysql_tables
{
	...
	do
		...
		mysqldump --single-transaction --quick --force --max_allowed_packet=512M --host=$DB_HOST --port=$DB_PORT --user=$DB_USER --password=$DB_PASSWORD $databaseName $tableName | tee pipe | gzip > $dumpFilepath
		rm pipe
...
function backup_mysql_database
{
	...
	mysqldump --single-transaction --quick --force --max_allowed_packet=512M --host=$DB_HOST --port=$DB_PORT --user=$DB_USER --password=$DB_PASSWORD $databaseName | tee pipeDatabase | gzip > $dumpFilepath
	rm pipeDatabase
	wait $PIDOF_SUCCESS_CHECK
